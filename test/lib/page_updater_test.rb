...
module Structure
  class PageUpdaterTest < ActiveSupport::TestCase
    ...
    def test_update_ancestors_if_parent_has_been_changed
      ...
      assert_nil @policy_page.permalink
      assert_nil @licence_page.permalink
      company_page = create(:page, title: 'Company', slug: 'company', permalink: 'company')

      subject.for(@information_page).update({ parent_id: company_page.id })

      reload_pages

      assert_equal 'company/information/about', @about_page.permalink
      assert_equal 'company/information/policy', @policy_page.permalink
      assert_equal 'company/information/policy/licence', @licence_page.permalink
    end

    def test_update_self_permalink_if_parent_has_been_changed
      @information_page.update({ permalink: 'information' })
      subject.for(@licence_page).update({ parent: @information_page })

      reload_pages

      assert_equal 'information/licence', @licence_page.permalink
    end
    # End of Tests
