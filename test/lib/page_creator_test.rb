...
module Structure
  class PageCreatorTest < ActiveSupport::TestCase
    ...
    def test_for_initializer
      assert_instance_of PageCreator, subject.for({})
    end

    def test_create_new_pages
      assert_instance_of Page, subject.for(valid_attr).create
    end

    def test_create_with_passed_attributes
      a_time = Time.zone.now
      a_page = subject.for(title: 'Catalog', visible: true, published_at: a_time).create
    ...
    def test_do_not_create_if_attr_is_not_valid
      a_page = subject.for(invalid_attr).create
      refute a_page.valid?
      refute a_page.persisted?
    end

    def test_set_permalink_from_slug_for_root_pages
      a_page = subject.for(title: 'About').create
    ...
    def test_set_permalink_from_parent_permalink_and_self_slug
      ...
      a_page = subject.for(title: 'Mission', parent: about_page).create
    ...
    def test_set_permalink_for_deep_structure
      ...
      a_page = subject.for(title: 'Algorithms', parent: deep_page).create
